...
# ╔═╡ 3fd467f6-20a6-4567-bd2e-a0fa7a161778
function loadIndexInd(idx, src)
	# Loading data
	per = (idx == "oni") & (src == "obs") ? "1982-2021" : "1981-2016"
	fil = datapath * "index/" * idx * "-" * src * "_" * per * "-mon.csv"
	dat = CSV.File(fil) |> DataFrame
	dat = dat[dat.year .<= 2016,:]
	
	# Getting a specific month
	if idx == "oni"
		dat = dat[dat.month .== 7,:]
	else
		dat = dat[6 .<= dat.month .<= 8,:]
		dat = src == "mem" ? groupby(dat, [:member, :year]) : groupby(dat, :year)
		dat = combine(dat, :idx => mean => :idx)
	end
	
	# Normalizing data
	if src != "mem"
		dat = normalize(dat.idx)
	else
		dat = groupby(dat, :member)
		dat = combine(dat, :year, :idx => (x -> normalize(x)) => :nor)
	end
...
# Precipitation - GPCP
begin
	pre_obs_nc = Dataset(datapath * "prec/gpcp_pre_1981-2016-JJAS_ts.nc")
...
begin
	pre_mem_nc = Dataset(datapath * "MR30/mr30-run_1981-2017_precip_JJAS.nc")
	pre_mem_old = pre_mem_nc["precip"][1,1,:,1:(end-1)]
...
begin
	oni_mem = loadIndexInd("oni", "mem")
	oni_obs_nor = loadIndexInd("oni", "obs")
...
begin
	dmi_obs_nor = loadIndexInd("dmi", "obs")
	dmi_mem = loadIndexInd("dmi", "mem")
end;
...
begin
	wio_obs_nor = loadIndexInd("wio", "obs")
	wio_mem = loadIndexInd("wio", "mem")
end;
...
begin
	wyi_obs_nor = loadIndexInd("wyi", "obs")
	wyi_mem = loadIndexInd("wyi", "mem")
end;
...
# ╔═╡ e30c1c21-7355-43cf-add8-968ae458c524
begin
	atl_obs_nor = loadIndexInd("atl", "obs")
	atl_mem = loadIndexInd("atl", "mem")
end;
...
# ╔═╡ 3c399ce2-524b-4686-b392-08f7b57947b3
g_atl = indexComp(
	obs  = atl_obs_nor,
	mem  = atl_mem,
	tit = "Normalized Atlantic anomalies",
	scale = true);
...
begin
	...
	for i in 1:nyear
		sel_pha_oni = subPhase(oni_mem[oni_mem.year .== tiempo[i],"nor"], oni_obs_nor[i])
		sel_pha_dmi = subPhase(dmi_mem[dmi_mem.year .== tiempo[i],"nor"], dmi_obs_nor[i])
		sel_pha_wio = subPhase(wio_mem[wio_mem.year .== tiempo[i],"nor"], wio_obs_nor[i])
		sel_pha_wyi = subPhase(wyi_mem[wyi_mem.year .== tiempo[i],"nor"], wyi_obs_nor[i])
		sel_pha_atl = subPhase(atl_mem[atl_mem.year .== tiempo[i],"nor"], atl_obs_nor[i])
	
		# Combining
		sel_pha_bth = sel_pha_wio .& sel_pha_oni
		#sel_pha_bth = sel_pha_oni .& sel_pha_dmi .& sel_pha_wyi
		...
		pre_mem_sel_pha[sel[sel_pha_atl] .* 36 .+ i,"atl"] .= true
	end
end;
...
begin
	g_sub_ens = subsamPlot(
		mem = per_pre_sel,
		name = "ens",
		tit = "Ensemble mean precipitation prediction for JJAS (30 members)",
		lgd = false)
	g_sub_prf = subsamPlot(
		mem = per_pre_sel,
		name = "sel",
		tit = "Perfect precipitation prediction for JJAS (10 members)",
		lgd = :outerbottom)
	
	#l_subsam = @layout[a b c; d e f]
	l_sub_ref = @layout[a{0.43h}; b]
	g_sub_ref = plot(
		g_sub_ens,
		g_sub_prf,
		layout = l_sub_ref)
...
# ╔═╡ 82515b34-1006-42af-b2bc-2750a5adf7eb
savefig(g_sub_ref, "../img/subsamp/ref_sub.png")
...
# ╔═╡ c35fbf60-61b5-406e-9a2c-b8dd365af689
g_sub_atl
...
begin
	#l_subsam = @layout[a b c; d e f]
	l_subsam = @layout[a b c; d{0.315w} e{0.47w}]
	g_subsam = plot(
		#g_sub_ens,
		g_sub_oni,
		g_sub_dmi,
...
savefig(g_subsam, "../img/subsamp/prf_sub_pha.png")
...
begin
	...
	for i in 1:nyear
		sel_clo_oni = subClose(oni_mem[oni_mem.year .== tiempo[i],"nor"], oni_obs_nor[i], n = nclo)
		sel_clo_dmi = subClose(dmi_mem[dmi_mem.year .== tiempo[i],"nor"], dmi_obs_nor[i], n = nclo)
		sel_clo_wio = subClose(wio_mem[wio_mem.year .== tiempo[i],"nor"], wio_obs_nor[i], n = nclo)
		sel_clo_wyi = subClose(wyi_mem[wyi_mem.year .== tiempo[i],"nor"], wyi_obs_nor[i], n = nclo)
		sel_clo_atl = subClose(atl_mem[atl_mem.year .== tiempo[i],"nor"], atl_obs_nor[i], n = nclo)
	
		# Combining
		sel_clo_all = sel_clo_oni .& sel_clo_wio # 10 sel
		...
		pre_mem_sel_clo[sel[sel_clo_atl] .* 36 .+ i,"atl"] .= true
	end
end;
...
# ╔═╡ 1cd06f8f-14f2-4f0c-b0d5-489b3fb5173d
g_sub_atl_clo;
...
begin
	...
	l_sub_clo = @layout[a b c; d{0.315w} e{0.47w}]
	g_sub_clo = plot(
		#g_sub_ens,
		g_sub_oni_clo,
		g_sub_dmi_clo,
...
savefig(g_sub_clo, "../img/subsamp/prf_sub_clo.png")
...
begin
	...
	secon_sel = 5
	for i in 1:nyear
		fil_eli = subClose(wio_mem[wio_mem.year .== tiempo[i],"nor"], wio_obs_nor[i], n = first_sel)
		...
		sel_eli_oni = subClose(oni_mem_fil, oni_obs_nor[i], n = secon_sel)
		sel_eli_dmi = subClose(dmi_mem_fil, dmi_obs_nor[i], n = secon_sel)
		sel_eli_wio = subClose(wio_mem_fil, wio_obs_nor[i], n = secon_sel)
		sel_eli_wyi = subClose(wyi_mem_fil, wyi_obs_nor[i], n = secon_sel)
...
begin
	#l_sub_eli = @layout[a b c; d{0.31w} e{0.31w}]
	l_sub_eli = @layout[a{0.43w} b{0.44w} c; d{0.41w} e]
	g_sub_eli = plot(
		#g_sub_ens,
		g_sub_eli_idx[1],
		g_sub_eli_idx[2],
		plot(grid = false, foreground_color_subplot = :white, legend = false),
		g_sub_eli_idx[3],
		g_sub_eli_idx[4],
		layout = l_sub_eli)
	#title!("Anomalies of precipitation for JJAS")
...
savefig(g_sub_eli, "../img/subsamp/prf_sub_eli.png")
