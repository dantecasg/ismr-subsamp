...
# ╔═╡ cf5b325e-e76c-4d3f-b06c-e1fb12926cd1
using NCDatasets

# ╔═╡ 5236cf71-4cd7-4568-88f5-572fcbd50e8a
using Plots

# ╔═╡ 7b4b9080-bc70-4e04-a3ab-af0a9b94e700
using Shapefile

# ╔═╡ e847cabd-5ecd-46f8-9e4b-dfb1fe3c1628
using DataFrames

# ╔═╡ aedd521f-ea0c-4754-81cf-7f3aff0b5e12
using Statistics

# ╔═╡ 8bd02a0f-8189-4a88-a876-89bf8a9cf93c
using StatsPlots

# ╔═╡ d718fb07-9028-4489-b5a1-ccd460f717c7
include("funcs_plot.jl");

# ╔═╡ 2c505416-c833-11eb-2a62-ebd3881fddd5
md"""
# Climatological analysis

Precipitation analysis for the region of India.

## Libraries
"""

# ╔═╡ 43d70858-ef75-412b-9df1-68c4a0a27127
md"## Functions"

# ╔═╡ 4e2cdb39-a8b4-45d2-9e1c-b855642cc790
md"""
### Plotting functions
"""

# ╔═╡ f19e68da-4741-4dc3-b163-620b50c40861
...
# ╔═╡ 14b0a38b-9d1f-422f-ae71-fcc4d1f09236
...
# ╔═╡ 9819df08-4181-47b4-b817-5137cfd6393f
md"""
### Data processing functions
"""

# ╔═╡ 5f91ab64-5fef-4dba-9d90-8c22e456f7b4
...
# ╔═╡ 6ea80c05-1b9f-419f-9e68-eed1a714e4ca
md"""
### Other functions
"""

# ╔═╡ 1cba72ed-413c-4dea-97fd-f304d8c85822
md"""
`funcs_plot.jl` contains the following functions:

- shp4plot -> Prepares shape file for plotting
"""

# ╔═╡ a00680af-4b95-403c-9978-4f922c6b7171
md"## Parameters"

# ╔═╡ 144ff9ee-e89c-42b1-acee-9e030cd726e0
datapath = "../../../data/";

# ╔═╡ ad211977-30b4-4414-bb57-0bf8d9305201
area = (20, 120, -20, 50);

# ╔═╡ be345408-3da3-4271-ace9-89e2bce36541
years = 1981:2016;

# ╔═╡ 3590c766-71b8-461c-8bc7-703414c005fb
coast_df = shp4plot("/home/paiche/Data/shapes/ne_110m_coastline.shp");

# ╔═╡ 4e3cefa0-a180-4719-90cf-a6375e77c62a
# Plot 2 spatial variables for 2 data sources
# Requires `statSpatial`
...
# ╔═╡ 22508ae2-7e08-4e17-9191-31a2944b30e4
md"""
## Spatial analysis
"""

# ╔═╡ 6dc94a96-b626-412b-94c5-d5684a0658f3
begin
	# GPCP data
	obs_cli_nc = Dataset(datapath * "prec/gpcp_pre_cli-JJAS.nc");
	obs_cli_mean = getVar(obs_cli_nc, var = "mean", coord = area)
	obs_cli_std  = getVar(obs_cli_nc, var = "std", coord = area)
	obs_lon, obs_lat = getVar(obs_cli_nc, var = "lonlat", coord = area)
end;

# ╔═╡ f3e6163e-03f1-41f8-8048-ac3d33cf75b7
begin
	# Assimilation data
	asm_cli_nc = Dataset(datapath * "prec/mpi-echam-asm_pre_cli-JJAS.nc");
	asm_cli_mean = getVar(asm_cli_nc, var = "mean", coord = area)
	asm_cli_std  = getVar(asm_cli_nc, var = "std", coord = area)
	asm_lon, asm_lat = getVar(asm_cli_nc, var = "lonlat", coord = area)
end;

# ╔═╡ 7ebbc807-e068-4b44-96b0-961830f777a6
md"""
### Plot
"""

# ╔═╡ a880a568-6b85-4cec-9afc-3323a43227d2
g_spt_clim = plotCompSpatial(
	data1 = (obs_cli_mean, obs_cli_std), 
	data2 = (asm_cli_mean, asm_cli_std), 
	coord1 = (obs_lon, obs_lat), 
	coord2 = (asm_lon, asm_lat), 
	xtit = ("GPCP", "Assimilation"), 
	ytit = ("Climatology", "Std. deviation"))

# ╔═╡ a6cd29f7-ba19-4e0c-965a-050099ecbc06
savefig(g_spt_clim, "../img/clim/spt_clim.png")

# ╔═╡ e0caf954-ede3-4b12-8a14-af403502261e
md"""
## Monthly climatology and JJAS anomalies
"""

# ╔═╡ 5827595a-70e5-458d-bace-0d65963d105e
# Monthly climatology
begin
	obs_cli_mon_nc = Dataset(datapath * "prec/gpcp_pre_cli-mon_ts.nc")
...
# ╔═╡ 44ee030f-5120-43c6-8ef6-7dff815b2f07
# JJAS anomalies time series
begin
	obs_jjas_ts_nc = Dataset(datapath * "prec/gpcp_pre_1981-2016-JJAS_ts.nc")
...
# ╔═╡ c716ac08-f38c-435c-8fd3-fa7cd54dbad6
...
# ╔═╡ 1a74ab74-7bbf-4dc4-bbee-58d5163dc2be
md"""
### Plot
"""

# ╔═╡ 2735eb80-71c3-4bc5-b34a-50cfd072eb37
g_prec_clim = plot(
...
# ╔═╡ 6048c01c-271d-4292-9dbd-890b8267c2b1
begin
	g_prec_anom = plot(
		years,
...
# ╔═╡ fea9ed78-b657-494a-9ab3-c433f5aa057d
# Exporting figure 2
begin
...
# ╔═╡ 0dfc945d-3cb6-430d-9500-86928207534f
